...
import { getnewsChannels,getnewsTags,editNews,getDetailNews } from "../../api/api";
...
export default {
  ...
  data() {
    return {
      ...
      uploadimage:process.env.HTTP.UPLOAD, 
      content: '',
      editorOption: {},
      channelName:[],
      options: [],
  ...
  methods: {
    ...
    getDetailNew(){
     ...
     getDetailNews(para).then(res=>{
     var data=JSON.parse(res.data.data).data.data;
     this.newsForm.title=data.title;
     this.newsForm.author=data.author;
     this.newsForm.keywords=data.keywords;
     this.newsForm.channelName=data.channelName;
      this.newsForm.weight=data.weight;
     this.newsForm.comments=data.comments;
     data.detail_tags.map((item)=>{
       this.dtags.push(item.name);
       this.newsForm.detail_tags= this.dtags;
     });
    this.newsForm.content=data.content;
    this.fileImage=data.image.split(',');
    this.fileImage.map((item,index)=>{
    this.fileImage1.url=this.fileImage[index];
    this.fileImage2.push(this.fileImage1);
    })
    this.fileList2=this.fileImage2;
     })
    },
    //表单提交
    submitSave(newsForm) {
  ...
  created() {
      this.id=Number(util.getQueryStringByName(this.id).id);
      this.getnewsChannel();
      this.getnewsTag();
      this.getDetailNew();
